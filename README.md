...
### v1.5.0 新增
- 🚦 **API 速率限制**：内存滑动窗口，普通接口 60次/分钟，AI 接口 10次/分钟
- 📦 **结果缓存**：化验单评估和症状分析结果 LRU 缓存（MD5 哈希键，最大 100 条）
- 🕐 **知识库时间戳**：条目自动添加 created_at / updated_at
- 🔧 **Docker 多阶段构建**：减小最终镜像体积
- 🛡️ **统一错误处理**：前端 axios 拦截器统一处理 HTTP 错误

### v1.6.0 新增
- 🧪 **单元测试**：128 个 pytest 测试用例，覆盖模型验证、知识库、对话历史、化验单解析、症状自查
...
- **v1.6.0 (2026-05-27)** - 质量保障版本
  - ✅ 添加 128 个 pytest 单元测试（模型/知识库/对话历史/化验单/症状自查）
  - ✅ 添加 pytest-cov 覆盖率报告支持
  - ✅ 添加 httpx 测试 HTTP 客户端

- **v1.5.0 (2026-05-27)** - 性能优化版本
