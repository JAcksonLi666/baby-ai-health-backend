...
### 新增功能 (v1.3.0)
- 🍼 **喂养记录管理**：母乳、配方奶、辅食、喝水记录
- 📏 **生长发育记录**：体重、身高、头围、体温记录
- 📊 **儿童生长发育数据库**：0-12岁 WHO/中国标准，支持百分位计算
...
| POST | /api/growth | 创建生长发育记录 |
| GET | /api/growth/{id} | 获取单条生长发育记录 |
| GET | /api/growth/latest | 获取最新记录 |
...
| 方法 | 路径 | 描述 |
|------|------|------|
| GET | /api/growth/standards | 获取生长标准值 |
| GET | /api/growth/percentile | 计算百分位 |
| GET | /api/growth/age-groups | 获取年龄段定义 |
| GET | /api/growth/metrics | 获取可用指标 |
...
- **v1.3.0 (2026-05-20)** - 数据收集增强版本
  - ✅ 添加喂养记录管理（母乳/配方奶/辅食/喝水）
  - ✅ 添加生长发育记录管理（体重/身高/头围/体温）
  - ✅ 添加儿童生长发育数据库（0-12岁 WHO/中国标准）
  - ✅ 添加生长发育百分位计算 API
  - ✅ 添加 Docker 部署配置
  - ✅ 添加群晖 NAS 部署支持
  - ✅ 添加 PWA 支持配置
